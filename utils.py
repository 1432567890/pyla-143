...
def refresh_brawl_stars_api_token_if_enabled(config, file_path="cfg/brawl_stars_api.toml", force=False):
    ...
    key_name_prefix = str(config.get("key_name_prefix", "Pyla 143 Auto")).strip() or "Pyla 143 Auto"
    ...
    description = str(config.get("key_description", "Auto-generated by Pyla 143 for the current public IP."))
    create_payload = {
        "name": key_name,
...
def save_dict_as_toml(data, file_path):
    with _toml_write_lock:
        path_dir = os.path.dirname(file_path) or "."
        os.makedirs(path_dir, exist_ok=True)
        fd, tmp_path = tempfile.mkstemp(prefix=f".{os.path.basename(file_path)}.", suffix=".tmp", dir=path_dir)
        try:
...
def save_brawler_data(data):
    ...
    """
    with _brawler_data_lock:
        fd, tmp_path = tempfile.mkstemp(prefix=".latest_brawler_data.", suffix=".tmp", dir=".")
        try:
            with os.fdopen(fd, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=4)
            os.replace(tmp_path, "latest_brawler_data.json")
        finally:
            if os.path.exists(tmp_path):
                os.remove(tmp_path)


def load_saved_brawler_data(file_path="latest_brawler_data.json"):
    try:
        ...
        print(f"Could not load saved brawler data from {file_path}: {e}")
        return []
